...
namespace Raytracer
{
  ...
  Color Texture::acquire_color(double x, double y, bool filter)
  {
    if (filter)
    {
      double a = floor(x) + 1 - x;
      double b = floor(y) + 1 - y;
      double c = x - floor(x);
      double d = y - floor(y);
      double w1 = a * b;
      double w2 = c * b;
      ...
      double w4 = c * d;
      int cx = (int)x;
      int dx = (cx < w - 1) ? cx + 1 : cx;
      int cy = (int)y;
      int dy = (cy < h - 1) ? cy + 1 : cy;
      int offset1 = 3 * (cy * w + cx);
      int offset2 = 3 * (dy * w + cx);
      int offset3 = 3 * (cy * w + dx);
      int offset4 = 3 * (dy * w + dx);
