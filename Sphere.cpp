...
namespace Raytracer
{
  ...
  Object(ka_r, ka_g, ka_b, kd_r, kd_g, kd_b, ks_r, ks_g, ks_b, ns, k_reflect,
  ...
  {
    if (texture->is_empty) return Color(1, 1, 1);
    Vector texture_pos = map_to_texture(position);
  ...
  Vector Sphere::map_to_texture(Vector position)
  {
    ...
    {
      p = p.normalize();
      u = (side.cross(p).dot(top)>0?
          acos(side.dot(p)):
          2*PI-acos(side.dot(p)))/(2*PI);
    }
    return Vector(u, v, 0);
  }
};
